...
const NavBar = () => {
  ...
  justify-between py-4 px-6">
        ...
        <div>
          ...
          <div className="flex items-center space-x-3">
            {!isAuthenticated ? (
              <Link 
                to="/login" 
                className="bg-black text-white px-4 py-1.5 rounded-md hover:bg-gray-800 transition text-base font-medium"
              >
                Login
              </Link>
            ) : (
      ...
      {/*Mobile Navigation */}
      <div
        className={`fixed top-0 text-base bottom-0 left-0 h-full w-3/4 sm:w-1/2 md:w-1/3 bg-white border-t shadow-lg z-50 transform transition-transform duration-300 
        ...
        <div className="p-3">
          ...
          <nav className="space-y-3">
            ...
            <Link
              ...
              className={`block text-gray-600 hover:text-black text-xs ${isActiveGender("Men") ? "font-bold bg-green-200 py-1.5 px-3 rounded-lg" : "py-1.5 px-3"}`}
            >
              Men
            </Link>
            <Link
              to="/collections/all?gender=Women"
              onClick={toggleNavDrawer}
              className={`block text-gray-600 hover:text-black text-xs ${isActiveGender("Women") ? "font-bold bg-green-200 py-1.5 px-3 rounded-lg" : "py-1.5 px-3"}`}
            >
              Women
            </Link>
            <Link
              to="/collections/all?category=Top Wear"
              onClick={toggleNavDrawer}
              className={`block text-gray-600 hover:text-black text-xs ${isActiveCategory("Top Wear") ? "font-bold bg-green-200 py-1.5 px-3 rounded-lg" : "py-1.5 px-3"}`}
            >
              Top Wear
            </Link>
            <Link
              to="/collections/all?category=Bottom Wear"
              onClick={toggleNavDrawer}
              className={`block text-gray-600 hover:text-black text-xs ${isActiveCategory("Bottom Wear") ? "font-bold bg-green-200 py-1.5 px-3 rounded-lg" : "py-1.5 px-3"}`}
            >
              Bottom Wear
            </Link>
          </nav>
